"""
Pyth-Sched-Analytics • v4.0 (Enterprise-Ready)
==============================================================
Includes: API-key authentication, rate limiting, input validation,
request-ID tracking, structured JSON logging, audit trail,
API versioning, CORS controls, Redis caching, NetworkKit
acceleration, sparse matrices, and vectorized operations.
"""
...
# ---------------------------------------------------------------------------
# Configuration (from centralised Config)
# ---------------------------------------------------------------------------
Config.validate()

SMALL_GRAPH_THRESHOLD = Config.SMALL_GRAPH_THRESHOLD
MAX_PATTERN_NODES = Config.MAX_PATTERN_NODES
MAX_PATTERNS = Config.MAX_PATTERNS
CACHE_SIZE = Config.CACHE_SIZE
REQUEST_TIMEOUT = Config.REQUEST_TIMEOUT
DEBUG = Config.DEBUG
ENABLE_SILHOUETTE_OPTIMIZATION = Config.ENABLE_SILHOUETTE_OPTIMIZATION
...
app.after_request(append_request_id_header)
app.after_request(append_rate_limit_headers)
app.after_request(audit_response)
...
def set_cached_result(key, value, ttl=None):
    ...
    if redis_client:
        try:
            ...
            if DEBUG:
                logger.info(f"Cached result in Redis: {key[:8]}...")
        except Exception as e:
            if DEBUG:
...
def detect_repeating_patterns(nodes_df):
    ...
    pattern_key = (sample_df['TaskType'].cat.codes.astype(int) * 100000 +
                  duration_discrete * 100 +
                  sample_df['Resources'].cat.codes.astype(int))
...
def serialize_work_packages(work_packages):
    ...
    for key, package in work_packages.items():
        try:
            ...
            if DEBUG:
                logger.warning(f"Error serializing work package {key}: {e}")
            serialized_packages[key] = {
...
def _dependency_groups_big(G_nx: nx.DiGraph, df: pd.DataFrame):
    ...
    try:
        ...
        logger.warning(f"NetworkKit dependency clustering failed: {e}")
        return _dependency_groups_small(G_nx, df)
...
def _dependency_groups_small(G: nx.DiGraph, df: pd.DataFrame):
    ...
    try:
        ...
        logger.error(f"Dependency clustering failed: {e}")
        df['DependencyCluster'] = 0
...
def _centralities(G: nx.DiGraph, df: pd.DataFrame):
    ...
    if _NK and len(G) > 50:
        try:
            ...
            logger.warning(f"NetworkKit centrality computation failed: {e}")
...
def _community_detection(G: nx.DiGraph, df: pd.DataFrame):
    ...
    if _NK and len(G) > 100:
        try:
            ...
            if DEBUG:
                logger.warning(f"NetworkKit community detection failed: {e}")
...
def _handle_graph_metrics():
    """Shared handler for graph-metrics (used by both legacy and v1 routes)."""
    ...
    # --- Input validation ---
    valid, err = validate_graph_payload(data)
    if not valid:
        return jsonify({
            'error': err,
    ...
    if cached_result:
        ...
        cached_result['request_id'] = getattr(g, 'request_id', None)
    ...
    try:
        ...
        if DEBUG or elapsed > 5:
            logger.info(f"Graph processed in {elapsed:.2f}s (nodes: {len(nodes)}, links: {len(links)})")
        res['processing_time'] = elapsed
        res['request_id'] = getattr(g, 'request_id', None)
...
def graph_metrics():
    return _handle_graph_metrics()


# Versioned route
@app.route('/api/v1/graph-metrics', methods=['POST', 'OPTIONS'])
def graph_metrics_v1():
    return _handle_graph_metrics()
...
def index():
    ...
    return jsonify({
        'status': 'ok',
        'service': 'python-sched-analytics',
        'version': 'v4.0',
    }), 200
...
def payload_too_large(e):
    ...
    return jsonify({
        'error': f'Payload exceeds maximum size of {Config.MAX_CONTENT_LENGTH_MB} MB',
